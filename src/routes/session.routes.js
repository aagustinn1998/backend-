...
export default class SessionRouter {
  ...
  initSessionRoutes() {
    ...
    this.router.post(`${this.path}/login`, async (req, res, next) => {
      ...
      try {
        ...
        if (!signUser) {
          ...
          return res.status(400).json({ message: "Correo electrónico o contraseña no válidos" });
        }

        // Genera un token JWT y establece una cookie para la sesión
    ...
    this.router.get(
      ...
      async (req, res, next) => {
        try {
          // Registra la desconexión del usuario y elimina la cookie de sesión
    ...
    // Eliminación de usuario (esto es para fines de prueba)
    ...
    this.router.post(`${this.path}/register`, async (req, res, next) => {
      try {
        ...
        if (user) {
          req.logger.warning(`El usuario ${email} ya existe`);
